...
/********int_to_string(<integer_value>): Converts integer to string*******/	
void int_to_string(unsigned int val)
{	
		if(val>99)                                              //Hundreds Place
		{
			lcd_write_char(0x30+(val/100));
		}
		if(val>9)                                               //Tens Place
		{
			lcd_write_char(0x30+(val%100/10));
		}
		lcd_write_char(0x30+val%10);                            //Unit Place
}

/********lcd_cmd(<char command>): Sends 8 bit command LCD display parameters*******/	
...
/********lcd_write_char(<char data>): Sends 8 bit character(ASCII) to be printed on LCD*******/	
...
/*******lcd_write_string(<string pointer>): Prints string on LCD. Requires string pointer as input argument.*******/	
